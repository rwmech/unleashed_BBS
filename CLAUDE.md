...
- **Board linking, DDial style** (Rob, to be designed when the terminal mode plugin is). Link two boards and the chat room spans both, so a quiet board borrows company from a busier one and neither needs more nodes of its own. That is what DDial did and it is the feature that made small systems worth calling.
  ...
  **Rob, 2026-09-25: the "superchat".** Link not only µnleashed boards but Diversi-DIAL and GTalk-style systems too, combining groups of different boards into one chat. The site presents it as coming (roadmap, no date). When designed, find out what the surviving DDial/GTalk systems actually speak on a link before promising compatibility, and carry it over the routing middleware's protocol.
