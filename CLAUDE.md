...
- 16 caller nodes (6 until 0.17.0), a busy line session (the caller past the last node: detection, busy screen, 10 s countdown), a hidden sysop node. Overflow callers get `BUSY` and a drop. Socket budget 24: every caller holds one, and the listener, mDNS and the backup window want theirs.
...
- **Use the agents.** `.claude/agents/` holds six: `bbs-regression` and `web-regression` before a commit, `bbs-qa` and `web-qa` when a change is visible to a caller or a reader, `code-review` on anything non-trivial, and `docs` when a change needs its documentation updated or a file has drifted from the code. They exist because each one encodes a bug that already shipped here, and running them is cheaper than finding those bugs the way they were found the first time.
...
## Current state (0.17.0 Block B, host-tested, not flashed)
