...
- SD card plugin (planned): takes over file management and lets logging be redirected to the card. It does **not** take the accounts: `users.txt` stays on `userdata` because it is the one thing that has to survive a card failing, and LittleFS is power-fail safe in a way FAT is not. Core file access must stay behind `plat::fsBase()` / `plat::logsBase()` so the plugin can swap the mounts.
...
- **Use the agents.** `.claude/agents/` holds five: `bbs-regression` and `web-regression` before a commit, `bbs-qa` and `web-qa` when a change is visible to a caller or a reader, `code-review` on anything non-trivial. They exist because each one encodes a bug that already shipped here, and running them is cheaper than finding those bugs the way they were found the first time.
...
## Current state (0.17.0 Block A, flashed by Rob 2026-09-19)
