...
| `MEM` | `M` | Heap statistics and session sizing, and the free space on the board's storage and card. The heap figures are read fresh every time. The storage figures are **kept** (1.1.2): measuring them walks the whole partition, which on a board held every caller for as long as 170 ms, so they are measured in the background at boot and at every staff login and shown from what was kept. A kept figure ends in `.`, and one line under them says when they were taken: `Figures ending in . are as of 14:02.` (`Ending in .: as of 14:02.` at 40 columns). A partition the board has just filled in bulk (a restore, `SCREENS INSTALL`) is measured again at the next staff login. `MEM FORCE` (staff) measures now, in the background, with a spinner, then draws MEM; for anybody else it says it is for staff. |
...
| `HARDWARE` | `HW` | What the board runs on (1.1.1), for every caller, guests included: the chip and its revision (`ESP32-D0WDQ6` `rev 1.0`, `ESP32-S3` `rev 0.2`), the CPU clock as it is running now and the number of cores, the flash, the PSRAM (the chip's size, and how much of it is mapped when that is less; `none` without), the board profile and its version with the board's name (`ESPCAM 1.0.1` `AI-Thinker ESP32-CAM`, or `ESP32 (reference)`), and `Capabilities`: what this image has and has running, a card and its size and bus (`32 GB SD card (SPI)`, or `SD slot (SDMMC), empty` on a board with a slot and no card in), a camera and its sensor once the sensor has answered and while a card is in (the camera needs one), the LCD panel, the LED lights once a pin is wired. Staff (co-sysop 2 and up) also see PSRAM free, internal heap free and its lowest, and card free (as the board last measured it). It never shows the network: address, Wi-Fi and signal stay in `SYS`. Every figure is a register or counter read. At 40 columns a board name that does not fit beside its row goes on the line under it, and the capabilities wrap under their label. |
...
The call warnings reach a caller wherever they are (1.1.2). At a prompt they read `5 minutes left.`; inside the chat room, the forums, the file areas or the mailbox they come in the board's own voice with the bell, `--> 5 minutes left on this call`, the way a page does; at a screen's `Press SPACE to continue` the line goes under it and the question again. A form shows them on its status line. Inside a plugin a caller is never idle (watching is not idling), so the idle warning has nowhere else to go.
...
| `SD MOUNT` | sysop | Mount the card without rebooting. **Pauses the whole board** for a few hundred milliseconds while it negotiates over SPI, which is why it is typed rather than retried on a timer. Says `Mounted: SDHC, a 30,436 MB card.`: the card's size, from its own register. How much of it is free is measured in the background afterwards (1.1.2), since counting a FAT card's free space reads its whole allocation table. |
| `SD UNMOUNT` | sysop | Flush and release, so the card can be pulled safely. Screens fall back to the stock set. The card stays out until `SD MOUNT`, a change of its pins in `CONFIG sd`, or a restart: a `CONFIG` save does not put it back (1.1.0). Refused while the board is working on the card in the background (`The board is using the card. Try again in a moment.`, or the camera's own words while a photo is being taken). |
| `UNBAN a.b.c.d` | `UNBAN` | Lift a ban. |
| `SCREENS` | any staff | Every screen by name, one row each: the size of each of `.ans`, `.asc` and `.seq`, and where callers get that copy from. At 80 columns that reads `flash`, `card, seeded` (the stock copy the board put on the card) or `card, own` (one the sysop edited or imported); at 40 it is `F`, `C` or `O`, with the key under the list. Read from the folders when asked, in the background with a spinner (1.1.2), and kept for a few seconds so `SCREENS` again, or `SCREENS INSTALL` straight after, does not read them twice. |
