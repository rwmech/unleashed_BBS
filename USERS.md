...
- `staff_at`, `staff_level` and `staff_ip` remember that this account entered a staff password, so it does not have to be typed on every call. Inside a week, logging in **from the same address** brings the level back automatically. The week is counted from the first time that day the password was typed (1.1.2): typing it again the same day from the same address does not rewrite `users.txt` just to move the date.
...
- A landing this board cannot do falls back to the main prompt without comment. That is deliberate: a board with chat switched off, or one that has no forums yet, should not greet somebody with an error because of a preference they set months ago.
