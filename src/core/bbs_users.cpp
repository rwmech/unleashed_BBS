 ...
 * Description: Accounts in the shell: the fill-in forms (signup, PROFILE,
...
void Bbs::formDone(Session& s, Color c, const char* msg) {
    ...
    if (s.backToUsers) {
        s.backToUsers = false;
        ulOpen(s);
        s.term.gotoXY(s.tl, 2, static_cast<uint8_t>(kListTop + ulRows(s) + 2));
...
void Bbs::formSave(Session& s, uint32_t now) {
    ...
    switch (s.formKind) {
        ...
        case FormKind::Profile: {
            ...
            users::Result r = users::update(s.user, s.edit);
        ...
        case FormKind::UserEdit: {
            ...
            s.edit.locked = s.yesno[0] == 'Y';
            if (*s.pwA) users::setPassword(s.edit, s.pwA);
            users::Result r;
            if (adding) {
                s.edit.created = clk::epoch();
                r = users::add(s.edit);
            } else {
                r = users::update(s.origHandle, s.edit);
            }
...
uint8_t Bbs::ulRows(const Session& s) const {
    ...
    uint8_t n = static_cast<uint8_t>(rows > kListTop + 4 ? rows - kListTop - 3 : 4);
...
void drawUserLine(Term& t, Timeline& tl, const UserRec& u, bool selected) {
    char buf[48];
    snprintf(buf, sizeof(buf), " %-13.13s %-16.16s %4u %c", u.handle, u.name,
...
void Bbs::ulKey(Session& s, int k, uint32_t now) {
    ...
    if (k == 'q' || k == 'Q' || k == KEY_ESC || k == KEY_BREAK) {
        t.reset(tl);
        t.cursor(tl, true);
        t.gotoXY(tl, 1, static_cast<uint8_t>(kListTop + h + 2));
        prompt(s);
        return;
    }
    if (k == 'a' || k == 'A') {
    ...
    if (k == KEY_ENTER || k == 'e' || k == 'E' || k == 'd' || k == 'D') {
        ...
        t.cursor(tl, true);
        if (k == 'd' || k == 'D') {
            t.gotoXY(tl, 2, static_cast<uint8_t>(kListTop + h + 2));
            if (ieq(s.origHandle, s.user)) {
                say(t, tl, Color::LightRed, "You cannot delete your own account.  ");
                return;
            }
...
bool Bbs::rowUsers(Session& s) {
    ...
    static UserRec u;
    if (users::at(static_cast<uint8_t>(i - 2), u)) {
        snprintf(buf, sizeof(buf), " %-13.13s %-16.16s %4u %c", u.handle, u.name,
                 static_cast<unsigned>(u.calls), u.locked ? 'L' : ' ');
