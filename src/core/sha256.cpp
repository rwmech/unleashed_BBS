 ...
 * ===========================================================================
 *
 * File:         src/core/sha256.cpp
 * Module:       Core / hashing
 *
 * Purpose:      SHA-256 (see sha256.h).
 *
 * Notes:        Plain FIPS 180-4 implementation, no heap, no platform dependencies.
 *
 * Libraries:    none
...
void Sha256::update(const uint8_t* data, size_t len) {
    total_ += len;
    while (len) {
        size_t take = 64u - fill_;
        if (take > len) take = len;
        memcpy(buf_ + fill_, data, take);
        fill_ = static_cast<uint8_t>(fill_ + take);
        data += take;
        len  -= take;
...
void Sha256::digest(const uint8_t* data, size_t len, uint8_t out[32]) {
    Sha256 s;
    s.update(data, len);
    s.finish(out);
}
